...
//     //   return;
//     // }
//     const image_base64 = image.image_base64 || '😩 missing';
//     const thumbnail_base64 = image.thumbnail || '😩 missing';

//     if (!image_base64 && !thumbnail_base64) {
//       worklist.setError("Image data is missing.");
//       return;
//     }
...
//     const base64 = image_base64.startsWith('data:') ? image_base64.split(',')[1] : image_base64;
//     const thumbnail = thumbnail_base64.startsWith('data:') ? thumbnail_base64.split(',')[1] : thumbnail_base64;
...
//             // languageResults.updateLanguageResult(language, 'isLoading', false);
//             // No need to set error state for abort, just stop loading
//             return;
//           }
...
//           languageResults.updateLanguageResult(language, 'isLoading', false);
//         } finally {
//           // Update progress regardless of success or failure (but not for abort if possible, though abort throws so we land here)
//           // If aborted, the loop continues but we might want to stop updating progress if controller changed.
...
//         <MiddlePanel
//           leftPanelView={leftPanelView}
//           className={`transition-all duration-300 ease-in-out ${isLeftPanelCollapsed ? 'md:flex-[3_1_0%]' : 'md:flex-1 md:max-w-[40vw] min-w-0'}`}
...
//           className={`transition-all duration-300 ease-in-out ${isLeftPanelCollapsed ? 'md:flex-[2_1_0%]' : 'md:flex-1'}`}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FFFFFF] via-[#E6F7FC] to-[#FDE6E0] text-gray-900 font-sans antialiased">
      ...
      <main className="flex-1 flex flex-col md:flex-row p-4 gap-4 min-h-[calc(100vh-80px)]">
        ...
        <MiddlePanel
          leftPanelView={leftPanelView}
          className={`transition-all duration-300 ease-in-out ${isLeftPanelCollapsed ? 'md:flex-[3_1_0%]' : 'md:flex-1 md:max-w-[33.33vw] min-w-0'}`}
          ...
          onReorderImagesOnPage={curriculum.reorderImagesOnPage}
          imageLoadingProgress={curriculum.imageLoadingProgress}
          onUpdateImageName={handleUpdateImageName} // PASSED HERE
        />
